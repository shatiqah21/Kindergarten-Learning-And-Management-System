...
class StudentRepo {
    ...
    public function exists($student_id)
    {
     ...
     /************* Transit & Daycare *************/
            public function findTransitStudents()
            {
                return $this->activeStudents()->where('addon_type', 'transit')->with(['user', 'my_class'])->get();
            }

            public function findDaycareStudents()
            {
                return $this->activeStudents()->where('addon_type', 'daycare')->with(['user', 'my_class'])->get();
            }
