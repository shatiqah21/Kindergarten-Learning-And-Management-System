...
class TimeTableController extends Controller
{
   ...
   public function parentIndex()
    {
        // cari semua anak untuk parent login
        $student_ids = \App\Models\StudentRecord::where('my_parent_id', auth()->id())->pluck('id');

        // cari semua class anak-anak tu melalui student_records
        $class_ids = \App\Models\StudentRecord::whereIn('user_id', $student_ids)
                    ->pluck('my_class_id');

        // ambil timetable ikut class anak
        $timetables = \App\Models\TimeTableRecord::whereIn('my_class_id', $class_ids)
                    ->with(['class','subject','teacher'])
                    ->get();

        return view('pages.parent.timetable.index', compact('timetables'));
    }
