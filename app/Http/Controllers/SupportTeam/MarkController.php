...
class MarkController extends Controller
{
    ...
    public function index()
    {
        ...
        $d['subjects'] = $this->my_class->getAllSubjects()->where('is_academic', 1);
        $d['selected'] = false;
    ...
    public function manage($exam_id, $class_id, $section_id, $subject_id)
    {
        ...
        $d['subjects'] = $this->my_class->getAllSubjects()->where('is_academic', 1);
      if (Qs::userIsTeacher()) {
            $d['subjects'] = \App\Models\Subject::where('my_class_id', $class_id)
                ->whereNotIn('name', ['Rehat', 'Perhimpunan Doa', 'Pulang'])
                ->orderBy('name')
                ->get();
        } else {
            $d['subjects'] = \App\Models\Subject::where('is_academic', 1)
                ->whereNotIn('name', ['Rehat', 'Perhimpunan Doa', 'Pulang'])
                ->orderBy('name')
                ->get();
        }
